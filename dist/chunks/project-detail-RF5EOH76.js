import{a as r,b as a}from"./chunk-HD4TFP6T.js";import{a as M}from"./chunk-ZUJQAZHO.js";import{f as N}from"./chunk-OO245FJT.js";var k=(i,n)=>i?`/.netlify/images?url=${encodeURIComponent(i)}&w=${n}&fm=webp&q=80`:"",F=i=>i?"AED\xA0"+Number(i).toLocaleString("en-AE",{maximumFractionDigits:0}):null,G=i=>i?i.replace(/<script[\s\S]*?<\/script>/gi,"").replace(/<iframe[\s\S]*?<\/iframe>/gi,"").replace(/\s+on\w+="[^"]*"/gi,"").replace(/\s+on\w+='[^']*'/gi,""):"",O=i=>i==="under_construction"?"Under Construction":i==="completed"?"Completed":"Off Plan",V=i=>/[/_-]thumb(nail)?[/_.-]/i.test(i);function W(i){let n=(i||"").toLowerCase();return/pool|swim/.test(n)?"pool":/gym|gymnasium|fitness/.test(n)?"fitness_center":/spa|sauna|steam/.test(n)?"spa":/tennis/.test(n)?"sports_tennis":/basketball|sport/.test(n)?"sports_basketball":/park|garden|landscap/.test(n)?"park":/beach|sea|waterfront/.test(n)?"beach_access":/security|guard|surveillance/.test(n)?"security":/parking|garage/.test(n)?"local_parking":/elevator|lift/.test(n)?"elevator":/concierge|reception|lobby/.test(n)?"concierge":/restaurant|dine|dining|cafe|food/.test(n)?"restaurant":/retail|shop|mall|store/.test(n)?"shopping_bag":/balcony|terrace/.test(n)?"balcony":/pet/.test(n)?"pets":/kids|child|play/.test(n)?"child_care":/storage/.test(n)?"storage":/smart|iot/.test(n)?"home_iot_device":/cctv|camera/.test(n)?"videocam":/bedroom|master/.test(n)?"bedroom_parent":"check_circle"}var w=[],j=0,h=1;function J(){if(document.getElementById("proj-lb"))return;let i=document.createElement("div");i.id="proj-lb",i.style.cssText="position:fixed;inset:0;z-index:9999;background:#000;display:none;flex-direction:column;align-items:stretch;",i.innerHTML=`
    ...
    </div>`,document.body.appendChild(i);let n=i.querySelector("#proj-lb-img"),l=!1,d=0,s=0;n.addEventListener("touchstart",e=>{e.touches.length===2?(l=!0,d=Math.hypot(e.touches[0].clientX-e.touches[1].clientX,e.touches[0].clientY-e.touches[1].clientY)):(s=e.touches[0].clientX,l=!1)},{passive:!0}),n.addEventListener("touchmove",e=>{if(l&&e.touches.length===2){let b=Math.hypot(e.touches[0].clientX-e.touches[1].clientX,e.touches[0].clientY-e.touches[1].clientY);h=Math.max(1,Math.min(4,h*(b/d))),d=b,n.style.transform=`scale(${h})`}},{passive:!0}),n.addEventListener("touchend",e=>{if(e.touches.length<2&&(l=!1),!l&&e.changedTouches.length===1&&h<=1.1){let b=e.changedTouches[0].clientX-s;Math.abs(b)>50&&window._lbStep(b<0?1:-1)}},{passive:!0})}function U(){let i=document.getElementById("proj-lb-img"),n=document.getElementById("proj-lb-counter"),l=document.getElementById("proj-lb-prev"),d=document.getElementById("proj-lb-next");if(!i)return;i.src=k(w[j],1200),i.style.transform=`scale(${h})`,n&&(n.textContent=`${j+1} / ${w.length}`);let s=w.length>1;l&&(l.style.display=s?"flex":"none"),d&&(d.style.display=s?"flex":"none")}window._lbStep=function(i){j=(j+i+w.length)%w.length,h=1,U()};window.openProjLightbox=function(i){J(),j=i,h=1;let n=document.getElementById("proj-lb");n.style.display="flex",document.body.style.overflow="hidden",U()};window.closeProjLightbox=function(){let i=document.getElementById("proj-lb");i&&(i.style.display="none"),document.body.style.overflow=""};async function ee(i){let n=document.getElementById("detail-sheet"),l=document.getElementById("detail-overlay");if(!n||!l)return;n.innerHTML=`
    ...
    <div style="text-align:center;padding:80px 24px;color:rgba(255,255,255,0.4);font-size:14px;">Loading project\u2026</div>`,l.classList.add("open"),document.body.style.overflow="hidden";let d=document.getElementById("detail-cta-bar");d&&(d.style.display="none");let s=document.getElementById("sticky-cta");s&&(s.dataset.prevDisplay=s.style.display,s.style.display="none");let{data:e,error:b}=await N.from("projects").select("slug,name,description,location,district_name,area,cover_image_url,min_price,max_price,min_area_sqft,max_area_sqft,completion_date,handover_percentage,payment_plan,payment_plan_detail,gallery_images,floor_plan_urls,available_units,facilities,nearby_locations,brochure_url,images_categorized,status,property_types,beds,developers(name,logo_url,website)").eq("slug",i).single();if(b||!e){n.innerHTML=`
      <button class="detail-back" onclick="closeDetail()" aria-label="Close">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M19 12H5"/><path d="M12 19l-7-7 7-7"/></svg>
      </button>
      <div style="text-align:center;padding:80px 24px;color:rgba(255,255,255,0.4);font-size:14px;">Project not found.</div>`;return}let c=e.developers||{},$=e.cover_image_url?k(e.cover_image_url,800):"",I=F(e.min_price),E=F(e.max_price),C=I&&E?`${I} \u2013 ${E}`:I||E||"",D=e.district_name||e.location||e.area||"",L=Array.isArray(e.property_types)&&e.property_types.length?e.property_types.join(", "):"",A=e.min_area_sqft&&e.max_area_sqft?`${Number(e.min_area_sqft).toLocaleString()} \u2013 ${Number(e.max_area_sqft).toLocaleString()} sqft`:e.min_area_sqft?`From ${Number(e.min_area_sqft).toLocaleString()} sqft`:"",g=e.images_categorized,p=[];if(g&&(g.interior?.length||g.exterior?.length))p=[...g.interior||[],...g.exterior||[]].filter(o=>o&&o!==e.cover_image_url);else if(Array.isArray(e.gallery_images)&&e.gallery_images.length){let t=new Set(g?.general||[]);p=e.gallery_images.filter(o=>o&&o!==e.cover_image_url&&!t.has(o))}p=p.filter(t=>!V(t)),w=e.cover_image_url?[e.cover_image_url,...p]:[...p];let q=g?.general?.filter(Boolean).length?g.general.filter(Boolean):Array.isArray(e.floor_plan_urls)?e.floor_plan_urls.filter(Boolean):[],z=($?1:0)+p.length,H=Array.isArray(e.facilities)&&e.facilities.length?e.facilities:[],B=Array.isArray(e.nearby_locations)&&e.nearby_locations.length?e.nearby_locations:[],T=e.available_units&&typeof e.available_units=="object"?Array.isArray(e.available_units)?e.available_units:e.available_units.units||[]:[],f=null,v=null,m=null,X=null,R=null,P=e.payment_plan_detail,u=Array.isArray(P)&&P.length>0?P[0]:null;if(u&&typeof u=="object"){X=u.title||null,R=Array.isArray(u.milestones)&&u.milestones.length?u.milestones:null;let t=u.heading_percentages;if(t&&typeof t=="object")for(let[o,y]of Object.entries(t)){let x=o.toLowerCase(),_=y?parseInt(String(y),10):null;x.includes("booking")?f=_:x.includes("construction")?v=_:(x.includes("completion")||x.includes("handover"))&&(m=_)}}else{let t=e.payment_plan;t&&typeof t=="object"&&!Array.isArray(t)?(f=t.booking??t.booking_percentage??null,v=t.construction??t.construction_percentage??null,m=t.handover??t.handover_percentage??null):e.handover_percentage!=null&&(m=e.handover_percentage,f=10,v=Math.max(0,100-f-m))}let Y=f!=null||v!=null||m!=null,S=e.completion_date?new Date(e.completion_date).toLocaleDateString("en-AE",{month:"long",year:"numeric"}):null;if(n.innerHTML=`
    <button class="detail-back" onclick="closeDetail()" aria-label="Close">
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M19 12H5"/><path d="M12 19l-7-7 7-7"/></svg>
    </button>
    ...
    <div class="detail-body" style="padding:20px 20px 80px;">
      ...
      <div style="margin-bottom:14px;">
        ...
        ${D?`<div style="font-size:13px;color:rgba(255,255,255,0.5);margin-top:4px;">\u{1F4CD} ${r(D)}</div>`:""}
      </div>
      ...
      <div style="display:flex;flex-wrap:wrap;gap:10px;margin-bottom:20px;">
        ...
        ${S?`<div style="background:rgba(255,255,255,0.06);border-radius:8px;padding:8px 12px;font-size:12px;"><span style="color:rgba(255,255,255,0.45);display:block;margin-bottom:2px;">Completion</span><span style="font-weight:600;">${r(S)}</span></div>`:""}
      </div>`:""}
      ...
      <div style="margin-bottom:20px;">
        ...
        <div style="display:flex;flex-direction:column;gap:8px;">
          ${T.map(t=>{let y=(t.bedroom?`${t.bedroom}BR `:"")+(t.property_types||"Unit"),x=t.lowest_area||t.area_sqft||t.area,_=t.lowest_price||t.price||t.min_price;return`
          <div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:10px 14px;">
            <div style="display:flex;justify-content:space-between;align-items:baseline;gap:8px;">
              <div style="font-size:13px;font-weight:600;">${r(y)}</div>
      ...
      <div style="margin-bottom:20px;">
        ...
        <div style="display:flex;flex-direction:column;gap:10px;">
          ${q.map((t,o)=>`<img src="${a(k(t,800))}" alt="Site plan ${o+1}" style="width:100%;border-radius:10px;background:rgba(255,255,255,0.04);" loading="lazy" onerror="this.style.display='none'">`).join("")}
        </div>
      </div>`:""}
      ...
      <div style="margin-bottom:20px;">
        ...
        <div style="display:flex;flex-direction:column;gap:0;">
          ${B.map((t,o)=>`
          <div style="display:flex;justify-content:space-between;align-items:center;padding:9px 0;${o<B.length-1?"border-bottom:1px solid rgba(255,255,255,0.06);":""}">
    ...
    <div style="display:flex;gap:8px;padding:12px 16px calc(12px + env(safe-area-inset-bottom));position:sticky;bottom:0;background:#000;border-top:1px solid rgba(255,255,255,0.06);">
      <button onclick="openLead('${a(e.name)}')" style="flex:1;padding:14px;background:#1127D2;border:none;border-radius:12px;color:#fff;font-size:14px;font-weight:600;font-family:'Inter',sans-serif;cursor:pointer;">Enquire</button>
    ...
    </div>`,z>1){let t=document.getElementById("proj-gallery"),o=document.getElementById("proj-gallery-count");t&&o&&t.addEventListener("scroll",()=>{let y=Math.round(t.scrollLeft/t.clientWidth);o.textContent=`${y+1} / ${z}`},{passive:!0})}}export{ee as openProjectDetail};
//# sourceMappingURL=project-detail-RF5EOH76.js.map
