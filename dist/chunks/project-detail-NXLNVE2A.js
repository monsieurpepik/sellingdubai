import{a as o,b as n}from"./chunk-HD4TFP6T.js";import{a as A}from"./chunk-ZUJQAZHO.js";import{f as M}from"./chunk-OO245FJT.js";var f=(r,g)=>r?`/.netlify/images?url=${encodeURIComponent(r)}&w=${g}&fm=webp&q=80`:"",C=r=>r?"AED\xA0"+Number(r).toLocaleString("en-AE",{maximumFractionDigits:0}):null,N=r=>r?r.replace(/<script[\s\S]*?<\/script>/gi,"").replace(/<iframe[\s\S]*?<\/iframe>/gi,"").replace(/\s+on\w+="[^"]*"/gi,"").replace(/\s+on\w+='[^']*'/gi,""):"",U=r=>r==="under_construction"?"Under Construction":r==="completed"?"Completed":"Off Plan";async function O(r){let g=document.getElementById("detail-sheet"),L=document.getElementById("detail-overlay");if(!g||!L)return;g.innerHTML=`
    ...
    <div style="position:relative;flex-shrink:0;">
      <div id="proj-gallery" style="height:240px;overflow-x:auto;overflow-y:hidden;scroll-snap-type:x mandatory;display:flex;background:#111;scrollbar-width:none;-webkit-overflow-scrolling:touch;">
        ${u?`<div style="flex:0 0 100%;scroll-snap-align:start;"><img src="${n(u)}" alt="${n(e.name)}" style="width:100%;height:240px;object-fit:cover;" loading="eager" onerror="handleImgError(this)"></div>`:""}
        ${y.map((t,i)=>`<div style="flex:0 0 100%;scroll-snap-align:start;"><img src="${n(f(t,800))}" alt="${n(e.name)} photo ${i+2}" style="width:100%;height:240px;object-fit:cover;" loading="lazy" onerror="handleImgError(this)"></div>`).join("")}
      </div>
      ${v>1?`<div id="proj-gallery-count" style="position:absolute;bottom:10px;right:10px;background:rgba(0,0,0,0.55);color:#fff;font-size:12px;font-weight:600;padding:4px 10px;border-radius:99px;pointer-events:none;">1 / ${v}</div>`:""}
    </div>`:""}
    ...
    <div class="detail-body" style="padding:20px 20px 80px;">
      ...
      <div style="margin-bottom:14px;">
        <span style="font-size:11px;font-weight:600;padding:3px 10px;border-radius:99px;background:rgba(255,255,255,0.1);color:rgba(255,255,255,0.7);margin-bottom:10px;display:inline-block;">${o(U(e.status))}</span>
      ...
      <div style="margin-bottom:20px;">
        ...
        <div style="display:flex;flex-direction:column;gap:8px;">
          ${B.map(t=>{let x=(t.bedroom?`${t.bedroom}BR `:"")+(t.property_types||"Unit"),c=t.lowest_area||t.area_sqft||t.area,m=t.lowest_price||t.price||t.min_price;return`
      ...
      <div style="margin-bottom:20px;">
        ...
        <div style="display:flex;flex-direction:column;gap:10px;">
          ${S.map((t,i)=>`<img src="${n(f(t,800))}" alt="Site plan ${i+1}" style="width:100%;border-radius:10px;background:rgba(255,255,255,0.04);" loading="lazy" onerror="this.style.display='none'">`).join("")}
        </div>
      </div>`:""}
      ...
      <div style="margin-bottom:20px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
          ...
          <div style="display:flex;align-items:center;gap:10px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.07);border-radius:12px;padding:10px;">
            ${t.image?`<img src="${n(f(t.image,80))}" alt="${n(t.name)}" style="width:48px;height:48px;object-fit:cover;border-radius:8px;background:rgba(255,255,255,0.06);flex-shrink:0;" loading="lazy" onerror="this.style.background='rgba(255,255,255,0.06)';this.style.display='none'">`:'<div style="width:48px;height:48px;border-radius:8px;background:rgba(255,255,255,0.06);flex-shrink:0;"></div>'}
      ...
      <div style="margin-bottom:20px;">
        ...
        <div id="proj-desc" style="font-size:13px;line-height:1.65;color:rgba(255,255,255,0.7);display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden;">${N(e.description)}</div>
    ...
    </div>`,v>1){let t=document.getElementById("proj-gallery"),i=document.getElementById("proj-gallery-count");t&&i&&t.addEventListener("scroll",()=>{let x=Math.round(t.scrollLeft/t.clientWidth);i.textContent=`${x+1} / ${v}`},{passive:!0})}}export{O as openProjectDetail};
//# sourceMappingURL=project-detail-NXLNVE2A.js.map
