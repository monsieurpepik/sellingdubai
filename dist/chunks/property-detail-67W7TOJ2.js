import{a as T}from"./chunk-U6DZRGHU.js";import"./chunk-XRWAOUFY.js";import{a as x}from"./chunk-2DHAKTSV.js";import{a as n,b as w}from"./chunk-HD4TFP6T.js";import{a as p,b as P}from"./chunk-ZUJQAZHO.js";import"./chunk-OO245FJT.js";var B=null;window.openPropertyById=function(t){let r=P.find(o=>String(o.id)===String(t));if(!r)return;B=r,z(r);let s=document.getElementById("detail-cta-bar");s&&(s.style.display=""),document.getElementById("detail-overlay").classList.add("open"),document.body.style.overflow="hidden",x("link_click",{link_type:"property_detail",property:r.title})};window.openPropertyDetail=function(t){let s=T()[t];if(!s)return;B=s,z(s);let o=document.getElementById("detail-cta-bar");o&&(o.style.display=""),document.getElementById("detail-overlay").classList.add("open"),document.body.style.overflow="hidden",x("link_click",{link_type:"property_detail",property:s.title})};window.closeDetail=function(){document.getElementById("detail-overlay").classList.remove("open");let t=document.getElementById("prop-overlay")?.classList.contains("open");document.body.style.overflow=t?"hidden":"",B=null,p&&history.pushState(null,"","/a/"+p.slug)};function z(t){window._currentProperty=t;let r=document.getElementById("detail-sheet"),s=t.additional_photos||[],o=[t.image_url,...s].filter(Boolean),d="";if(o.length>0){let i=`<img id="detail-hero-img" class="detail-hero" src="${w(o[0])}" alt="${w(t.title)}" loading="lazy" onclick="openPhotoViewer(window._currentDetailHeroIdx||0)" style="cursor:pointer" onerror="handleImgError(this)">`;if(o.length>1){let e=o.slice(1,5).map((u,f)=>`<img src="${w(u)}" alt="" loading="lazy" onclick="swapDetailHero(${f+1})" style="width:100%;aspect-ratio:4/3;object-fit:cover;display:block;cursor:pointer" onerror="handleImgError(this)">`).join("");d=`<div class="detail-gallery-wrap">${i}<div class="detail-gallery">${e}</div><button class="detail-show-all" onclick="openFullGallery()"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg>SHOW ALL PHOTOS</button></div>`}else d=i}window._currentDetailImages=o,window._currentDetailHeroIdx=0;let a=t.title?`<div class="detail-breadcrumb">${n(t.title)}</div>`:"",b=t.title?`<div class="detail-title-above">${n(t.title)}</div>`:"",M="";if(t.price){let i=n(t.price);M=/AED/i.test(i)?i:`AED ${i}`}let g=[];t.bedrooms&&g.push(`<div class="detail-spec"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M3 7v11a1 1 0 001 1h16a1 1 0 001-1V7"/><path d="M21 11H3V9a2 2 0 012-2h14a2 2 0 012 2v2z"/></svg>${t.bedrooms} Bed${t.bedrooms>1?"s":""}</div>`),t.bathrooms&&g.push(`<div class="detail-spec"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M4 12h16v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5z"/><path d="M6 12V5a2 2 0 012-2h1a2 2 0 012 2v1"/></svg>${t.bathrooms} Bath${t.bathrooms>1?"s":""}</div>`),t.area_sqft&&g.push(`<div class="detail-spec"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 3v18"/></svg>${t.area_sqft.toLocaleString()} sqft</div>`);let S=g.length>0?`<div class="detail-specs-row">${g.join('<div class="detail-spec-divider"></div>')}</div>`:"",h="",v=[];t.property_type&&v.push(`<div class="detail-info-item"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M3 21h18"/><path d="M5 21V7l8-4v18"/><path d="M19 21V11l-6-4"/><path d="M9 9h1m-1 4h1m-1 4h1"/></svg>${n(t.property_type)}</div>`),t.land_area&&v.push(`<div class="detail-info-item"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="0" stroke-dasharray="4 2"/></svg>${t.land_area} m\xB2 Land</div>`);let m=(t.features||[]).map(i=>{let e=i.toLowerCase(),l='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>';return e.includes("view")||e.includes("landmark")||e.includes("panoram")?l='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="M2 8h20"/><circle cx="12" cy="14" r="3"/></svg>':e.includes("pool")?l='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M2 15c3 0 3-2 6-2s3 2 6 2 3-2 6-2"/><path d="M2 19c3 0 3-2 6-2s3 2 6 2 3-2 6-2"/><circle cx="8" cy="8" r="2"/><path d="M16 8h-4l-2 3"/></svg>':e.includes("gym")||e.includes("fitness")?l='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M6.5 6.5h11M6.5 17.5h11"/><rect x="2" y="8" width="4" height="8" rx="1"/><rect x="18" y="8" width="4" height="8" rx="1"/><path d="M6.5 12h11"/></svg>':e.includes("garden")?l='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 22V12"/><path d="M7 12c0-2.76 2.24-5 5-5s5 2.24 5 5"/><path d="M4 15c0-3.31 3.58-6 8-6s8 2.69 8 6"/></svg>':e.includes("jacuzzi")||e.includes("spa")?l='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M4 12h16v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5z"/><path d="M8 7c0-1 .5-2 2-2s2 1 2 0 .5-2 2-2 2 1 2 2"/></svg>':e.includes("bbq")||e.includes("barbecue")?l='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="10" r="6"/><path d="M12 16v4"/><path d="M8 20h8"/><path d="M9 7c1 1 2 1 3 0s2-1 3 0"/></svg>':e.includes("parking")||e.includes("garage")?l='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M9 17V7h4a3 3 0 010 6H9"/></svg>':e.includes("balcony")&&(l='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M3 15h18"/><path d="M3 15v5h18v-5"/><path d="M9 15V5h6v10"/></svg>'),`<div class="detail-info-item">${l}${n(i)}</div>`});(v.length>0||m.length>0)&&(h='<div class="detail-info-card">',v.length>0&&(h+=`<div class="detail-info-title">Property Details</div><div class="detail-info-row">${v.join("")}</div>`),v.length>0&&m.length>0&&(h+='<div class="detail-info-divider"></div>'),m.length>0&&(h+=`<div class="detail-info-title">Amenities</div><div class="detail-info-row">${m.join("")}</div>`),h+="</div>");let V=t.description?`<div class="detail-description-card"><div class="detail-section-title">Description</div><div class="detail-description">${n(t.description)}</div></div>`:"",C="";if(t.location){let i=encodeURIComponent(t.location+", Dubai, UAE"),e=`https://www.google.com/maps/search/?api=1&query=${i}`;C=`<div class="detail-location-card"><div class="detail-section-title">Location</div>
    ...
    </div>`);let D="";if(t.price){let i=parseFloat(String(t.price).replace(/[^0-9.]/g,""));if(i>0){let e=i*.04,l=i>=5e5?4200:2100,u=580,f=i*.021,E=2500,y=e+l+u+f+E,I=i*.8,L=I*.0025+290,H=I*.01,q=y+L+H,c=$=>"AED "+Math.round($).toLocaleString(),A=$=>($*100).toFixed(1)+"%";D=`
    ...
    <div class="detail-title-bar">${a}${b}</div>
    ${d}
  ...
  `,r.scrollTop=0;let k=document.getElementById("detail-wa-btn"),O=document.getElementById("detail-inquire-btn");p&&p.whatsapp?(k.style.display="flex",k.onclick=()=>{window.open(`https://wa.me/${p.whatsapp.replace(/[^0-9]/g,"")}?text=${encodeURIComponent("Hi, I'm interested in: "+(t.title||"your property"))}`,"_blank"),x("whatsapp_tap",{source:"property_detail",property:t.title})}):k.style.display="none",O.onclick=()=>openLeadForProperty(t.title)}window.toggleCostMode=function(t,r){t.parentElement.querySelectorAll(".cost-toggle-btn").forEach(b=>b.classList.remove("active")),t.classList.add("active");let s=document.getElementById("cost-mortgage-rows"),o=document.getElementById("cost-total-value"),d=document.getElementById("cost-total-pct");if(!s||!window._costData)return;let a=window._costData;r==="mortgage"?(s.classList.remove("hidden"),o.textContent=a.fmtAED(a.mortgageTotal),d.textContent=a.fmtPct(a.mortgageTotal/a.rawPrice)):(s.classList.add("hidden"),o.textContent=a.fmtAED(a.cashTotal),d.textContent=a.fmtPct(a.cashTotal/a.rawPrice))};
//# sourceMappingURL=property-detail-67W7TOJ2.js.map
