  ...
  try {
    ...
    if (!customerId) {
      ...
      if (!customerRes.ok) {
        ...
        return new Response(JSON.stringify({ error: "Failed to create billing customer." }), { status: 502, headers: cors });
      }
    ...
    if (!sessionRes.ok) {
      ...
      return new Response(JSON.stringify({ error: "Failed to create checkout session." }), { status: 502, headers: cors });
    }
