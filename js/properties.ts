...
// PROPERTY STATUS MAP
// ==========================================
export const STATUS_MAP: Record<string, { label: string; css: string }> = {
...
function injectDemoPhotos(p: Property): Property {
  if (!DEMO_MODE) return p;
  const seed = (p.title || '').length + (p.id || '').length;
  const result = { ...p };
  const existingPhotos = Array.isArray(result.additional_photos) ? result.additional_photos as string[] : [];
  if (existingPhotos.length === 0) {
    const count = 4 + (seed % 3);
    const shuffled = [...DEMO_PHOTOS].sort((a, b) =>
      Math.sin(seed * 9301 + DEMO_PHOTOS.indexOf(a)) - Math.sin(seed * 7919 + DEMO_PHOTOS.indexOf(b))
    );
    result.additional_photos = shuffled.slice(0, count) as unknown as typeof result.additional_photos;
  }
  const existingFeatures = Array.isArray(result.features) ? result.features as string[] : [];
  if (existingFeatures.length === 0) {
    result.features = (DEMO_AMENITIES[seed % DEMO_AMENITIES.length] ?? DEMO_AMENITIES[0]) as unknown as typeof result.features;
  }
  ...
  if (!result.property_type) {
    ...
    result.property_type = types[seed % types.length] ?? 'Villa';
  }
  const extResult = result as Property & { land_area?: number | null };
  if (!extResult.land_area && (result.property_type || '').toLowerCase().includes('villa')) {
    extResult.land_area = 3000 + (seed * 500 % 8000);
  }
  return result;
}
...
export async function loadProperties(agentId: string): Promise<Property[]> {
  if (propertiesLoaded && propertiesAgentId && agentId !== propertiesAgentId) {
    // Different agent — reset all pagination state
...
export async function loadMoreProperties(): Promise<Property[]> {
  if (!propertiesHasMore || !propertiesAgentId || propertiesLoadingMore) return [];
  propertiesLoadingMore = true;
  try {
...
// ==========================================
export function renderPropertyList(props: Property[]): void {
...
// ==========================================
export function initOffPlanCarousel(carouselId = 'offplan-carousel', dotsId = 'offplan-dots'): void {
  const carousel = document.getElementById(carouselId) as (HTMLElement & { _carouselInit?: boolean }) | null;
  const track = carousel?.querySelector<HTMLElement>('.offplan-track');
  ...
  const cards = track.querySelectorAll<HTMLElement>('.offplan-card');
  ...
  function snapToCard(idx: number): void {
    if (idx < 0) idx = 0;
...
function renderRemProjectCard(p: RemProject, devName: string): string {
...
export async function loadRemProjects(_agentSlug: string, agentId: string): Promise<void> {
  try {
    ...
    const { data } = await supabase
      ...
      .eq('status', 'approved')
      .limit(12);
    projects = ((data || []).map(row => (row as { projects: RemProject | null }).projects).filter(Boolean)) as RemProject[];
