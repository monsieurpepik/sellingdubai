...
function renderDetailView(p: Property): void {
  ...
  const extras = (p.additional_photos ?? []) as string[];
  const allImages = ([p.image_url, ...extras] as Array<string | null | undefined>).filter((x): x is string => Boolean(x));
  ...
  const landArea = (p as Property & { land_area?: number | null }).land_area;
  if (landArea) detailItems.push(`<div class="detail-info-item"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="0" stroke-dasharray="4 2"/></svg>${landArea} m² Land</div>`);

  const amenityItems = ((p.features ?? []) as string[]).map(f => {
  ...
  if (inquireBtn) inquireBtn.onclick = () => window.openLeadForProperty!(p.title ?? '');
}
...
window.toggleCostMode = function(btn: HTMLElement, mode: string) {
  ...
  if (mode === 'mortgage') {
    ...
    if (totalEl) totalEl.textContent = d.fmtAED(d.mortgageTotal);
    if (pctEl) pctEl.textContent = d.fmtPct(d.mortgageTotal / d.rawPrice);
  } else {
    mortRows.classList.add('hidden');
    if (totalEl) totalEl.textContent = d.fmtAED(d.cashTotal);
    if (pctEl) pctEl.textContent = d.fmtPct(d.cashTotal / d.rawPrice);
  }
};
